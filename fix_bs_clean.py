...
# Extract mobile nav block from ceramica (clean source)
nav_start = ceramica.find('\n<nav class="mobile-nav"')
nav_end   = ceramica.find('\n<script src="cart.js">', nav_start)
nav_block = ceramica[nav_start:nav_end]
# Fix the "active" link to best-sellers
nav_block = nav_block.replace(
    'colecao-best-sellers.html" class="bottom-nav-item active"',
    'colecao-best-sellers.html" class="bottom-nav-item active"')
...
new_css = '''
    ...
    .bs-cat-label { font-size: 12px; font-weight: 800; color: #fe5000; letter-spacing: 1px; text-transform: uppercase; margin-bottom: 8px; }
...
# ── Rebuild: take head+CSS, strip all BS CSS, inject fresh, keep header/nav ──

# 1. Find the ORIGINAL </style> before any BS CSS was added
#    (it's the only </style> in the current clean file)
style_close_idx = html.find('  </style>\n</head>')
if style_close_idx == -1:
    print('ERROR: no </style> found'); exit(1)

# 2. Find the end of </header>
header_end_idx = html.find('</header>\n') + len('</header>\n')
if header_end_idx < len('</header>\n'):
    print('ERROR: no </header> found'); exit(1)

# The head section (up to and NOT including the existing BS CSS)
# We want: everything from start up to the first </style> marker,
# then inject new CSS, then close style+head
original_head_open = html[:style_close_idx]  # up to just before '  </style>'

# The nav/header section (between </head> and </header>\n)
head_close_idx = html.find('</head>\n') + len('</head>\n')
nav_header_part = html[head_close_idx:header_end_idx]
...
print(f'Done. File: {len(result)} chars, lines: {result.count(chr(10))}')
