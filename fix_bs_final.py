...
chaleiras_cards = '\n'.join([
    prod_card('Chaleira Clássica Azure','899,00 – R$ 1.299,00','chaleira-classica.html',
              CDN+'dwc412fcee/images/BestSeller/chaleira-tradiconal-azure.png'),
    prod_card('Chaleira Clássica','899,00 – R$ 1.299,00','chaleira-classica.html',
              CDN+'dw05bada9d/images/BestSeller/chaleira-cl%C3%A1ssica-com-apito-bamboo.png'),
    prod_card('Chaleira Clássica Shell Pink','899,00 – R$ 1.299,00','chaleira-classica.html',
              CDN+'dw265d0f03/images/BestSeller/chaleira-cl%C3%A1ssica-com-apito-shell%20pink.png'),
    prod_card('Chaleira Clássica Cotton','899,00 – R$ 1.299,00','chaleira-classica.html',
...
new_css = '''
    ...
    .bs-hero img { width: 100%; height: 100%; object-fit: cover; object-position: center 30%; display: block; }
    ...
    .bs-cat { max-width: 1400px; margin: 0 auto 72px; padding: 0 48px; }
    .bs-cat-label { font-size: 12px; font-weight: 800; color: #fe5000; letter-spacing: 1px; text-transform: uppercase; margin-bottom: 8px; display: flex; align-items: center; gap: 4px; }

    .bs-split { display: grid; grid-template-columns: 1fr 1fr; gap: 60px; align-items: center; margin-bottom: 40px; }
    .bs-split-reverse { direction: rtl; }
    .bs-split-reverse > * { direction: ltr; }
    .bs-circle-wrap { width: 100%; aspect-ratio: 1; border-radius: 50%; overflow: hidden; }
    .bs-circle-wrap img { width: 100%; height: 100%; object-fit: cover; display: block; }
    .bs-split-text h2 { font-size: 28px; font-weight: 800; margin-bottom: 16px; line-height: 1.2; }
    .bs-split-text p { font-size: 14px; color: #444; line-height: 1.7; margin-bottom: 12px; }
    .bs-split-text ul { list-style: none; padding: 0; margin: 0 0 20px; }
    .bs-split-text ul li { font-size: 13px; color: #444; line-height: 1.9; padding-left: 16px; position: relative; }
    .bs-split-text ul li::before { content: "✓"; position: absolute; left: 0; color: #fe5000; font-weight: 700; }
    .bs-ver-todos { font-size: 11px; font-weight: 800; letter-spacing: 2px; text-transform: uppercase; color: #000; border-bottom: 1.5px solid #000; padding-bottom: 2px; display: inline-block; margin-top: 4px; }
    .bs-ver-todos:hover { color: #fe5000; border-color: #fe5000; }

    .bs-products { display: flex; gap: 16px; overflow-x: auto; scroll-snap-type: x mandatory; -webkit-overflow-scrolling: touch; padding-bottom: 8px; scrollbar-width: thin; }
    .bs-products::-webkit-scrollbar { height: 3px; }
    .bs-products::-webkit-scrollbar-thumb { background: #ccc; border-radius: 2px; }
    .bs-prod-card { flex: 0 0 200px; scroll-snap-align: start; text-align: left; }
    ...
    .bs-prod-img img { width: 100%; height: 100%; object-fit: contain; padding: 10px; transition: transform 0.3s; display: block; }
    .bs-prod-card:hover .bs-prod-img img { transform: scale(1.07); }
    .bs-prod-name { font-size: 13px; font-weight: 600; line-height: 1.3; margin-bottom: 6px; color: #111; }
    .bs-prod-price { font-size: 13px; font-weight: 800; color: #111; margin-bottom: 4px; }
    .bs-prod-star { color: #fe5000; font-size: 11px; letter-spacing: 1px; }

    .bs-side-section { max-width: 1400px; margin: 0 auto 72px; padding: 0 48px; }
    .bs-side-head { font-size: 26px; font-weight: 800; margin-bottom: 32px; }
    .bs-side-split { display: grid; grid-template-columns: 1fr 1fr; gap: 40px; margin-bottom: 40px; align-items: start; }
    .bs-side-img img { width: 100%; height: 300px; object-fit: cover; display: block; }
    ...
    .bs-side-text ul { list-style: none; padding: 0; margin: 0 0 16px; }
    .bs-side-text ul li { font-size: 13px; color: #555; line-height: 1.9; padding-left: 16px; position: relative; }
    .bs-side-text ul li::before { content: "✓"; position: absolute; left: 0; color: #fe5000; font-weight: 700; }

    .bs-recipes { max-width: 1400px; margin: 0 auto 80px; padding: 0 48px; }
    .bs-recipe-head { text-align: center; margin-bottom: 36px; }
    .bs-recipe-head h2 { font-size: 28px; font-weight: 600; color: #333; margin-bottom: 4px; }
    .bs-recipe-head h3 { font-size: 28px; font-weight: 700; color: #fe5000; }
    ...
    .bs-recipe-img img { width: 100%; height: 100%; object-fit: cover; transition: transform 0.3s; display: block; }
    .bs-recipe-item:hover .bs-recipe-img img { transform: scale(1.05); }
    .bs-recipe-name { font-size: 14px; font-weight: 400; color: #333; margin-bottom: 14px; }
    .bs-recipe-btn { display: inline-block; padding: 9px 22px; font-size: 11px; font-weight: 700; letter-spacing: 1px; text-transform: uppercase; border: 1.5px solid #000; color: #000; background: #fff; transition: all 0.2s; }
    .bs-recipe-btn:hover { background: #000; color: #fff; }

    @media (max-width: 900px) {
      .bs-hero { height: 240px; }
      .bs-hero-overlay { padding: 0 0 30px 24px; }
      .bs-hero-title { font-size: 30px; }
      .bs-main-heading { font-size: 26px; margin-bottom: 40px; }
      .bs-split, .bs-side-split { grid-template-columns: 1fr; gap: 24px; }
      .bs-split-reverse { direction: ltr; }
      .bs-recipe-grid { grid-template-columns: repeat(2, 1fr); gap: 14px; }
      .bs-prod-card { flex: 0 0 150px; }
      .bs-cat, .bs-side-section, .bs-recipes { padding: 0 16px; margin-bottom: 48px; }
    }
'''
...
new_content = f'''<div class="bs-hero">
  <img src="{CDN2}dw1b33812b/images/BestSeller/4K-TIF-20231123_BAKINGACTI2024_044.jpg?sw=1440&sfrm=png" alt="Best-sellers Le Creuset" />
...
<div class="bs-illustration">
  <img src="{CDN2}dwb4230871/images/BestSeller/lp-best-seller.jpg?sw=768&sfrm=png" alt="Le Creuset ilustração" />
</div>
...
<!-- FERRO FUNDIDO -->
<div class="bs-cat">
  <div class="bs-cat-label">BEST-SELLER &#9733;</div>
  <div class="bs-split">
...
<div class="bs-side-section">
  ...
  <div class="bs-side-split">
    ...
    <div class="bs-side-text">
      ...
      <a href="colecao-ceramica.html" class="bs-ver-todos">VER TODOS &rsaquo;</a>
    </div>
  </div>
  <div class="bs-products">
{ceramica_cards}
...
<div class="bs-cat">
  ...
  <div class="bs-split bs-split-reverse">
    ...
    <div class="bs-split-text">
      ...
      <p>O pegador e o bico são resistentes ao calor, trazendo mais segurança no manuseio. O apito sonoro avisa quando a água atinge o ponto de fervura.</p>
      <ul>
      ...
      <a href="colecao-chaleiras.html" class="bs-ver-todos">VER TODOS &rsaquo;</a>
    </div>
  </div>
  <div class="bs-products">
{chaleiras_cards}
...
# ── Rebuild file from scratch keeping only head+nav, replacing body content ──

# Find the end of </header> which is our start marker
header_end = html.find('</header>\n') + len('</header>\n')

# Find the script tag that ends the page (before </body>)
end_marker = '\n<script src="cart.js">'
end_pos = html.find(end_marker)

if header_end == -1 or end_pos == -1:
    print(f'ERROR: header_end={header_end}, end_pos={end_pos}')
    exit(1)

# Strip old accumulated CSS blocks — keep only up to FIRST </style>
head_part = html[:header_end]

# Remove all accumulated BS CSS blocks (there may be multiple from failed runs)
# Find the one </style> that closes the main <style> block
style_close = head_part.find('  </style>\n</head>')
if style_close == -1:
    print('ERROR: could not find </style>')
    exit(1)

# Keep head up to just before </style>, inject fresh CSS, close it
clean_head = head_part[:style_close] + new_css + '  </style>\n</head>\n'

# Rebuild
# Everything after </head> until </header> is the nav/header HTML
nav_part = head_part[head_part.find('</head>\n') + len('</head>\n'):]

tail = html[end_pos:]  # \n<script src="cart.js">...</body></html>

result = clean_head + nav_part + new_content + tail

with open('colecao-best-sellers.html', 'w', encoding='utf-8') as f:
    f.write(result)

print(f'Done. File size: {len(result)} chars')
print(f'  header_end={header_end}, end_pos={end_pos}')
